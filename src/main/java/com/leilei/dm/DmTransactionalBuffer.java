 ...
 */
@NotThreadSafe
public final class DmTransactionalBuffer implements AutoCloseable {

    private static final Logger LOGGER = LoggerFactory.getLogger(DmTransactionalBuffer.class);
     ...
     */
    DmTransactionalBuffer(DmDatabaseSchema schema, Clock clock, ErrorHandler errorHandler, DmStreamingChangeEventSourceMetrics streamingMetrics) {
     ...
     */
    void registerDmlOperation(int operation, String transactionId, Scn scn, TableId tableId, DmLogMinerDmlEntry parseEntry, Instant changeTime, String rowId) {
        if (abandonedTransactionIds.contains(transactionId)) {
            DmLogMinerHelper.logWarn(streamingMetrics, "Captured DML for abandoned transaction {}, ignored.", transactionId);
            return;
        }
        if (rolledBackTransactionIds.contains(transactionId)) {
            DmLogMinerHelper.logWarn(streamingMetrics, "Captured DML for rolled back transaction {}, ignored.", transactionId);
            return;
        }
    ...
    private void commit(ChangeEventSource.ChangeEventSourceContext context, DmOffsetContext offsetContext, Instant start,
        ...
        try {
            ...
            for (DmlEvent event : transaction.events) {
                ...
                dispatcher.dispatchDataChangeEvent(event.getTableId(),
                        new DmLogMinerChangeRecordEmitter(
                                offsetContext,
                                event.getEntry(),
            ...
            DmLogMinerHelper.logError(streamingMetrics, "Thread interrupted during running", e);
    ...
    void abandonLongTransactions(Scn thresholdScn, DmOffsetContext offsetContext) {
        DmLogMinerHelper.logWarn(streamingMetrics, "All transactions with first SCN <= {} will be abandoned, offset: {}", thresholdScn, offsetContext.getScn());
        ...
        while (iter.hasNext()) {
            ...
            if (transaction.getValue().firstScn.compareTo(threshold) <= 0) {
                DmLogMinerHelper.logWarn(streamingMetrics, "Following long running transaction {} will be abandoned and ignored: {} ", transaction.getKey(),
    ...
    private static class DmlEvent {
        ...
        private final DmLogMinerDmlEntry entry;
        private final Scn scn;
        private final TableId tableId;
        private final String rowId;

        public DmlEvent(int operation, DmLogMinerDmlEntry entry, Scn scn, TableId tableId, String rowId) {
        ...
        public DmLogMinerDmlEntry getEntry() {
            return entry;
        }
