...
public class DmConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DmConnectorConfig.class);
    ...
    public static final Field LOG_MINING_DML_PARSER = Field.createInternal("log.mining.dml.parser")
            ...
            //TODO dm
            .withEnum(LogMiningDmlDmDmParser.class, LogMiningDmlDmDmParser.LEGACY)
            .withWidth(Width.SHORT)
            .withImportance(Importance.LOW)
    ...
    private final DmHistoryRecorder logMiningHistoryRecorder;
    ...
    private final LogMiningDmlDmDmParser dmlParser;
    ...
    public DmConnectorConfig(Configuration config) {
        ...
        this.dmlParser = LogMiningDmlDmDmParser.parse(config.getString(LOG_MINING_DML_PARSER));
    }
    ...
    private static DmHistoryRecorder resolveLogMiningHistoryRecorder(Configuration config) {
        if (!config.hasKey(LOG_MINING_HISTORY_RECORDER_CLASS.name())) {
            return new DmNeverHistoryRecorder();
        }
        return config.getInstance(LOG_MINING_HISTORY_RECORDER_CLASS, DmHistoryRecorder.class);
    }
    ...
    protected HistoryRecordComparator getHistoryRecordComparator() {
        return new HistoryRecordComparator() {
            ...
            protected boolean isPositionAtOrBefore(Document recorded, Document desired) {
                Scn recordedScn;
                Scn desiredScn;
                if (getAdapter() == DmConnectorConfig.ConnectorAdapter.XSTREAM) {
    ...
    public enum ConnectorAdapter implements EnumeratedValue {
        ...
        XSTREAM("XStream") {
            ...
            public String getConnectionUrl() {
                 //达梦
                return "jdbc:dm://${" + JdbcConfiguration.HOSTNAME + "}:${" + JdbcConfiguration.PORT + "}/";
    ...
    public enum LogMiningDmlDmDmParser implements EnumeratedValue {
        LEGACY("legacy"),
        FAST("fast");

        private final String value;

        LogMiningDmlDmDmParser(String value) {
            this.value = value;
        }
        ...
        public static LogMiningDmlDmDmParser parse(String value) {
            if (value == null) {
                return null;
            }
            value = value.trim();
            for (LogMiningDmlDmDmParser parser : LogMiningDmlDmDmParser.values()) {
        ...
        public static LogMiningDmlDmDmParser parse(String value, String defaultValue) {
            LogMiningDmlDmDmParser mode = parse(value);
            if (mode == null && defaultValue != null) {
                mode = parse(defaultValue);
            }
            return mode;
     ...
     */
    public DmHistoryRecorder getLogMiningHistoryRecorder() {
     ...
     */
    public LogMiningDmlDmDmParser getLogMiningDmlParser() {
        return dmlParser;
    }
