...
public class DmSqlUtils {
    ...
    //TODO dm V$LOG 是 V$ARCHIVED_LOG
    private static final String LOG_VIEW = "V$ARCHIVED_LOG";
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DmSqlUtils.class);
    ...
    //TODO dm V$ARCHIVED_LOG
    static String currentRedoLogSequenceQuery() {
        return String.format("SELECT SEQUENCE# FROM %s WHERE STATUS = 'A'", LOG_VIEW);
    }
    ...
    static String currentScnQuery() {
        // TODO  String.format("SELECT CURRENT_SCN FROM %s", DATABASE_VIEW) 替换为 select CUR_LSN from  V$RLOG
        return "select CUR_LSN from  V$RLOG";
    }
    ...
    public static String allOnlineLogsQuery() {
        //TODO  dm 獲取文件
        return String.format("SELECT MIN(L.NAME) AS FILE_NAME, L.NEXT_CHANGE# AS NEXT_CHANGE, '1' as GROUP#, L.FIRST_CHANGE# AS FIRST_CHANGE, L.STATUS  FROM V$ARCHIVED_LOG L\n" +
                "WHERE L.NEXT_CHANGE# > 0  GROUP BY  L.NEXT_CHANGE#, L.FIRST_CHANGE#, L.STATUS ORDER BY 3\n");
...
//                " GROUP BY F.GROUP#, L.NEXT_CHANGE#, L.FIRST_CHANGE#, L.STATUS ORDER BY 3", LOG_VIEW, LOGFILE_VIEW);
     ...
     * @return the query
     */
    static String logMinerContentsQuery(DmConnectorConfig connectorConfig, String logMinerUser) {
        StringBuilder query = new StringBuilder();
        //TODO  dm ROLLBACK 是 ROLL_BACK
