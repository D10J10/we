 ...
 */
class DmLcrEventHandler implements XStreamLCRCallbackHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(DmLcrEventHandler.class);
    ...
    private final DmXstreamStreamingChangeEventSource eventSource;

    public DmLcrEventHandler(ErrorHandler errorHandler, EventDispatcher<TableId> dispatcher, Clock clock, RelationalDatabaseSchema schema,
                             DmOffsetContext offsetContext, boolean tablenameCaseInsensitive, DmXstreamStreamingChangeEventSource eventSource) {
    ...
    private void dispatchSchemaChangeEvent(DDLLCR ddlLcr) throws InterruptedException {
        ...
        dispatcher.dispatchSchemaChangeEvent(
                tableId,
                new DmXStreamSchemaChangeEventEmitter(offsetContext, tableId, ddlLcr));
    }
    ...
    private void setWatermark() {
        ...
        try {
            final DmXstreamStreamingChangeEventSource.PositionAndScn message = eventSource.receivePublishedPosition();
            if (message == null) {
                return;
            }
