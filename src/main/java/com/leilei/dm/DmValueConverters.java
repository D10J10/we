...
public class DmValueConverters extends JdbcValueConverters {
    ...
    private static final DateTimeFormatter TIMES_FORMATTER = new DateTimeFormatterBuilder()
    ...
    public SchemaBuilder schemaBuilder(Column column) {
        ...
        switch (column.jdbcType()) {
            ...
            case DmdbType.BLOB:
                if (column.typeName().equalsIgnoreCase("BLOB")){
            ...
            default: {
                try {
                    ...
                    logger.warn("JdbcValueConverters returned exception '{}' for column '{}'", e.getMessage(), column.name());
                    return variableScaleSchema(column);
                }
            }
        }
    ...
    public ValueConverter converter(Column column, Field fieldDefn) {
        switch (column.jdbcType()) {
            ...
            case Types.DATE:
                // 达梦的DATE类型 精度是13(最大)
                if (adaptiveTimePrecisionMode || adaptiveTimeMicrosecondsPrecisionMode && (getTimePrecision(column) <= 13)) {
            ...
            case Types.VARBINARY:
                // 达梦的BLOB类型 精度是2147483647(最大)
                // 达梦的BINARY类型 精度是2147483647(最大)
                // 达梦的VARBINARY类型 精度是2147483647(最大)
    ...
    protected Object convertTest(Column column, Field fieldDefn, Object data) {
    ...
    protected Object convertIntervalDate(Column column, Field fieldDefn, Object data) {
        //     if (data instanceof String) {
//            final Matcher toTimestampTzMatcher = TO_DATE_.matcher((String) data);
//            if (toTimestampTzMatcher.matches()) {
//                String dateText = toTimestampTzMatcher.group(1);
//                data = ZonedDateTime.from(TIMESTAMP_TZ_FORMATTER.parse(dateText.trim()));
//            }
        //      }
//        data= ;
        return converter(column, fieldDefn);
        // return new java.util.Date();
    ...
    private void convertOracleIntervalDaySecond(Object data, ResultReceiver r) {
        ...
        if (m.matches()) {
            ...
            r.deliver(MicroDuration.durationMicros(
                    0,
                    0,
                    sign * Integer.parseInt(m.group(2)),
                    sign * Integer.parseInt(m.group(3)),
                    sign * Integer.parseInt(m.group(4)),
                    sign * Integer.parseInt(m.group(5)),
                    sign * Integer.parseInt(Strings.pad(m.group(6), 6, '0')),
                    MicroDuration.DAYS_PER_MONTH_AVG));
        }
    }

    /**
     * 重写convertBinary方法 使得达梦的BLOB类型 存的是16进制 精度是2147483647(最大)
     *
     * @param column    列
     * @param fieldDefn 字段定义
     * @param data      数据
     * @param mode      二进制编码模式
     * @return 字节
     */
    ...
    protected Object convertBinary(Column column, Field fieldDefn, Object data, BinaryHandlingMode mode) {
        if (data instanceof DmdbBlob) {
            // 达梦的BLOB类型 存的是16进制
            try {
                //转换成16进制
                data = ((DmdbBlob) data).do_getBytes(1, (int) ((DmdbBlob) data).do_length());
            } catch (SQLException e) {
                throw new DebeziumException("无法转换列的值:" + column.name(), e);
            }
        }
        if (data instanceof byte[]) {
            // 达梦的BINary类型 存的是16进制
    ...
    /**
     * 重写convertBit方法 使得达梦的BIT类型 精度是1
     *
     * @param column    列
     * @param fieldDefn 字段定义
     * @param data      数据
    ...
    protected Object convertTime(Column column, Field fieldDefn, Object data) {
        //达梦的TIME类型 精度是6
        if (String.valueOf(data).contains("TIME")){
            Pattern pattern = Pattern.compile("TIME '(.*)'");
            Matcher matcher = pattern.matcher(String.valueOf(data));
            if (matcher.find()) {
                String group = matcher.group(1);
                //将String 转TIME类型 TODO 有问题TIME 只支持到秒 不能支持到毫秒 去除毫秒
                if (group.contains(".")){
                    group=group.substring(0,group.indexOf("."));
                }
                data= Time.valueOf(group.trim());
            }
        }
    ...
    protected Object convertDateToEpochDays(Column column, Field fieldDefn, Object data) {
        ...
        //判断data 里是否有DATE
        if (String.valueOf(data).contains("DATE ")) {
            Pattern pattern = Pattern.compile("DATE '(.*)'");
            Matcher matcher = pattern.matcher(String.valueOf(data));
            if (matcher.find()) {
                String group = matcher.group(1);
                //将DATE '2021-01-01' 转换成 2021-01-01
                // 将字符串转换成日期
                data= LocalDate.from(TIMES_FORMATTER.parse(group.trim())).atStartOfDay();
            }
        }
        return super.convertDateToEpochDays(column, fieldDefn, data);
    }
    private Object interceptTimeStamp( Object data) {
        if(String.valueOf(data).contains("TIMESTAMP")){
