...
public class DmConnection extends  JdbcConnection {
     ...
     */
    private final DmDatabaseVersion databaseVersion;
    ...
    public DmDatabaseVersion getOracleVersion() {
        return databaseVersion;
    }

    private DmDatabaseVersion resolveOracleDatabaseVersion() {
        String versionStr;
        try {
            try {
                ...
                //TODO 修改将 SELECT BANNER_FULL FROM V$VERSION WHERE BANNER_FULL LIKE 'Oracle Database%' 改为Dm的  SELECT BANNER 	 FROM V$VERSION WHERE BANNER LIKE 'DM Database%'
                versionStr = queryAndMap("SELECT BANNER \t FROM V$VERSION WHERE BANNER LIKE 'DM Database%'", (rs) -> {
                    if (rs.next()) {
                        //TODO  rs.getString(1)  改为  Oracle Database 11g Enterprise Edition Release 11.2.0.4.0 - 64bit Production
                        return   "Oracle Database 11g Enterprise Edition Release 11.2.0.4.0 - 64bit Production";
                    }
                    return null;
                });
            }
...
//                "and (table_name NOT LIKE 'SYS_IOT_OVER_%' and IOT_NAME IS NULL) ";
        ...
        query(query, (rs) -> {
            while (rs.next()) {
                //TODO dm  rs.getString(1)  改为  OWNER
     ...
     */
    public boolean getTablenameCaseInsensitivity(DmConnectorConfig connectorConfig) {
    ...
    public DmConnection executeLegacy(String... sqlStatements) throws SQLException {
    ...
    public DmConnection executeLegacy(Operations operations) throws SQLException {
