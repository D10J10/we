 ...
 */
public class DmLogMinerDmlParser implements DmDmlParser {
    ...
    @Override
    public DmLogMinerDmlEntry parse(String sql, Table table, String txId) {
        if (table == null) {
     ...
     */
    private DmLogMinerDmlEntry parseInsert(String sql, Table table) {
        try {
            ...
            final List<DmLogMinerColumnValue> newValues = new ArrayList<>(columnNames.size());
            ...
            return new DmLogMinerDmlEntryImpl(Envelope.Operation.CREATE, newValues, Collections.emptyList());
        }
        catch (Exception e) {
     ...
     * @return the parsed DML entry record or {@code null} if the SQL was not parsed
     */
    private DmLogMinerDmlEntry parseUpdate(String sql, Table table) {
        try {
            ...
            // set before
            final List<DmLogMinerColumnValue> oldValues;
            ...
            return new DmLogMinerDmlEntryImpl(Envelope.Operation.UPDATE, newValues, oldValues);
        }
        catch (Exception e) {
     ...
     * @return the parsed DML entry record or {@code null} if the SQL was not parsed
     */
    private DmLogMinerDmlEntry parseDelete(String sql, Table table) {
        try {
            ...
            List<DmLogMinerColumnValue> oldValues;
            if (!beforeColumnMap.isEmpty()) {
            ...
            return new DmLogMinerDmlEntryImpl(Envelope.Operation.DELETE, Collections.emptyList(), oldValues);
        }
        catch (Exception e) {
    ...
    private int parseColumnValuesClause(String sql, int start, List<String> columnValues) {
        ...
        if (!sql.substring(index, index + 8).equalsIgnoreCase(VALUES)) {
    ...
    private int parseSetClause(String sql, int start, List<String> columnNames, List<String> columnValues) {
        ...
        if (!sql.substring(start, start + SET_LENGTH).equalsIgnoreCase(SET)) {
            throw new DebeziumException("Failed to parse DML: " + sql);
        }
        start += SET_LENGTH;
        ...
        for (; index < sql.length(); ++index) {
            ...
            if (c == '"' && inColumnName) {
                ...
                // TODO dm
                if ((c == 'w'||c=='W') && (lookAhead == 'h'||lookAhead=='H') && sql.substring(index - 1, index + WHERE_LENGTH - 1).equalsIgnoreCase(WHERE)) {
                    index -= 1;
                    break;
                }
            }
    ...
    private int parseWhereClause(String sql, int start, List<String> columnNames, List<String> columnValues) {
        ...
        if (!sql.substring(start, start + WHERE_LENGTH).equalsIgnoreCase(WHERE)) {
            throw new DebeziumException("Failed to parse DML: " + sql);
        }
        start += WHERE_LENGTH;
     ...
     */
    private static DmLogMinerColumnValue createColumnValue(String columnName, String columnValue) {
        DmLogMinerColumnValue value = new DmLogMinerColumnValueImpl(columnName, 0);
