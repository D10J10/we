...
import com.leilei.dm.DmConnector;
...
//        DM 的连接器配置
        ...
        return io.debezium.config.Configuration.create()
                //            连接器的Java类名称
                .with("connector.class", DmConnector.class.getName())
                .with("jdbc.driver", "dm.jdbc.driver.DmDriver")
                //            task
                .with("tasks.max", "1")
                //            偏移量持久化，用来容错 默认值
                ...
                .with("database.tablename.case.insensitive", "false")
                .with("bigint.unsigned.handling.mode", "long")
                ...
                // dm 数据库需要配置  jdbc:oracle:thin:@//${hostname}:${port}/${dbname}
              // .with("database.url", "jdbc:oracle:thin:@${hostname}:${port}/${dbname}")
                .with("database.url", "jdbc:dm://${hostname}:${port}/")
                .with("log.mining.dml.parser", "legacy")
                //                	oracle数据库服务器名称
